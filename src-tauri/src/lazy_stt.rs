//! Lazy STT server manager — starts the local faster-whisper STT server
...
/// Uses a raw TCP connection to avoid tokio runtime dependency — this
...
pub fn check_stt_idle() {
    ...
    if should_kill {
        tracing::info!("lazy_stt: STT server idle for {}s — killing to save RAM", STT_IDLE_TIMEOUT.as_secs());
